...
\hline
Date of Issue & January 25, 2026 \\
\hline
Document Status & Production-Ready (Lab / Proof-of-Concept) \\
\hline
...
\hline
1.0 & 2025-12-01 & Initial professional documentation of lab implementation & Your Name \\
\hline
1.1 & 2025-12-14 & Technical corrections, validation alignment, and terminology precision & Your Name \\
\hline
1.2 & 2026-25-01 & Raw data review and LaTeX formatting overhaul & Your Name \\
\hline
\end{tabular}
...
This document uses \textbf{hub-and-spoke WAN terminology} consistently:

\begin{itemize}
  \item \textbf{Hub}: Central aggregation nodes hosted in AWS EC2, terminating WireGuard tunnels and acting as \textbf{eBGP transit hubs} for this lab scope
...
This document describes a \textbf{fully functional lab implementation} designed to validate an enterprise-style WAN pattern:
...
This document describes the design, implementation, and validation of an \textbf{AWS-hosted hub-and-spoke WAN architecture} using \textbf{WireGuard} for encrypted transport, \textbf{BGP (FRRouting)} for dynamic routing, and \textbf{AWS-native automation} for active/standby high availability.

The environment is implemented as a \textbf{lab / proof-of-concept} but is intentionally designed to align with \textbf{enterprise-grade architectural principles}, including deterministic routing behavior, encrypted transport, automated failure handling, and operational observability. While not feature-complete relative to large-scale production WANs (e.g., active/active hubs, VRFs, or centralized SD-WAN controllers), all architectural decisions reflect realistic operational constraints and failure scenarios commonly encountered in small-to-medium enterprise deployments.
...
The WAN follows a \textbf{hub-and-spoke topology}:

\begin{itemize}
  \item Two AWS EC2 instances act as \textbf{eBGP transit hub nodes} (DC1-SP1 and DC1-SP2), configured as an active/standby pair.
  \item Multiple remote \textbf{spoke nodes} (SITE1-LF1, SITE2-LF1, SITE3-LF1) connect to the hubs over WireGuard tunnels.
...
High availability is achieved using \textbf{AWS-native event-driven automation} rather than traditional network-layer redundancy protocols.
...
Dynamic routing is handled exclusively via \textbf{eBGP} for overlay reachability:
...
\begin{itemize}
  ...
  \item No static routes are used for \textbf{overlay} (inter-site or spoke-to-hub) reachability; all overlay prefixes are learned and installed via BGP.
\end{itemize}
...
This document covers the design, deployment, and validation of a \textbf{hub-and-spoke WAN architecture} implemented using open standards and AWS-native services. The scope of the implementation includes:
...
The reference architecture implements a \textbf{hub-and-spoke WAN model} using encrypted point-to-point tunnels for transport and BGP for overlay routing. A pair of redundant hub nodes are deployed in AWS, while multiple spoke nodes represent remote sites.
...
All inter-site \textbf{overlay} communication is routed via the active hub. Spokes do not establish direct tunnels or routing adjacencies with each other.
...
\texttt{dist/diagrams/logical-architecture.svg}, and the document embeds the SVG so that both
HTML and PDF outputs are consistent and deterministic.
\end{quote}

\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{diagrams/logical-architecture.svg}
...
Hub redundancy is achieved through an \textbf{active/standby failover model} rather than protocol-level multi-homing.
...
Although the hostnames retain a \texttt{SP} / \texttt{LF} naming convention, the logical roles are defined as follows:
...
Each hub node hosts \textbf{three independent WireGuard interfaces}, one per spoke:
...
Each spoke hosts exactly \textbf{one WireGuard interface} (\texttt{wg0}) connected to the hub.
...
Both hub instances use \textbf{identical interface IP addressing}. At any given time, only the hub associated with the Elastic IP is reachable.
...
The two hub instances use \textbf{identical WireGuard configuration files}. The following configuration applies to both DC1-SP1 and DC1-SP2.
...
\begin{itemize}
  ...
  \item Section~\ref{sec:operational-considerations}
  \item Section~\ref{sec:failure-recovery}
\end{itemize}
...
\begin{verbatim}
ip addr show wg1
4: wg1: <POINTOPOINT,NOARP,UP,LOWER_UP> mtu 8921 qdisc noqueue state UNKNOWN group default qlen 1000
    link/none
...
\begin{verbatim}
ip addr show wg2
5: wg2: <POINTOPOINT,NOARP,UP,LOWER_UP> mtu 8921 qdisc noqueue state UNKNOWN group default qlen 1000
    link/none
...
\begin{verbatim}
ip addr show wg0
9: wg0: <POINTOPOINT,NOARP,UP,LOWER_UP> mtu 8921 qdisc noqueue state UNKNOWN group default qlen 1000
    link/none
    inet 10.100.1.1/30 scope global wg0
       valid_lft forever preferred_lft forever
\end{verbatim}
...
\begin{verbatim}
ip addr show wg1
10: wg1: <POINTOPOINT,NOARP,UP,LOWER_UP> mtu 8921 qdisc noqueue state UNKNOWN group default qlen 1000
    link/none
    inet 10.100.2.1/30 scope global wg1
       valid_lft forever preferred_lft forever
\end{verbatim}
...
\begin{verbatim}
ip addr show wg2
11: wg2: <POINTOPOINT,NOARP,UP,LOWER_UP> mtu 8921 qdisc noqueue state UNKNOWN group default qlen 1000
    link/none
    inet 10.100.3.1/30 scope global wg2
       valid_lft forever preferred_lft forever
\end{verbatim}
...
10.100.1.2      4      65101        18        16        0    0    0 00:00:21            1        4 SITE1-LF1
10.100.2.2      4      65102        17        15        0    0    0 00:00:18            1        4 SITE2-LF1
10.100.3.2      4      65103        19        17        0    0    0 00:00:16            1        4 SITE3-LF1
...
Neighbor        V         AS   MsgRcvd   MsgSent   TblVer  InQ OutQ  Up/Down State/PfxRcd   PfxSnt Desc
10.100.1.2      4      65101       181       174        0    0    0 00:25:09            1        4 SITE1-LF1
10.100.2.2      4      65102       118       115        0    0    0 00:25:22            1        4 SITE2-LF1
10.100.3.2      4      65103        46        39        0    0    0 00:24:58            1        4 SITE3-LF1
...
Neighbor        V         AS   MsgRcvd   MsgSent   TblVer  InQ OutQ  Up/Down State/PfxRcd   PfxSnt Desc
10.100.1.1      4      65000        15        15        0    0    0 00:06:18            3        4 DC1-SP1
...
Neighbor        V         AS   MsgRcvd   MsgSent   TblVer  InQ OutQ  Up/Down State/PfxRcd   PfxSnt Desc
10.100.2.1      4      65000        14        14        0    0    0 00:07:08            3        4 DC1-SP1
...
Neighbor        V         AS   MsgRcvd   MsgSent   TblVer  InQ OutQ  Up/Down State/PfxRcd   PfxSnt Desc
10.100.3.1      4      65000        14        14        0    0    0 00:07:45            3        4 DC1-SP1
...
\end{itemize}
Only one hub is intended to be \emph{externally active} at a time, defined as ``currently associated with the Elastic IP (EIP).''

\paragraph{Spoke behavior (no configuration changes during failover)}
All spokes terminate WireGuard sessions to the hub \emph{via the EIP} rather than instance-specific public IPs. This provides:
\begin{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{CloudWatch alarm (contingency + telemetry)}: triggers failover if DC1-SP1 is \emph{running but unhealthy}, and provides a native audit trail of health degradation.
\end{enumerate}
...
EC2 instance lifecycle state alone does not represent ``running but unhealthy.'' To cover this case, CloudWatch alarms are used to detect \emph{instance-level} health check failure while the instance remains in the \texttt{running} state.
...
The following areas are explicitly \emph{out of scope} for this lab:
\begin{itemize}
...
Key rotation is \textbf{manual and out of scope} for this lab. This is explicitly documented to avoid implying operational maturity beyond the intended proof-of-concept scope.
...
\begin{itemize}
  \item \textbf{DC1-SP1} is in the \emph{running} state and associated with the Elastic IP (EIP).
  \item \textbf{DC1-SP2} is in the \emph{running} state but has no EIP association.
...
CloudWatch alarms do \emph{not} trigger failback, by design.
...
A critical architectural principle validated by this lab is the strict separation between the \textbf{underlay} and \textbf{overlay} planes.
...
\begin{enumerate}
  ...
  Failover behavior is observable, auditable, and repeatable using standard AWS tooling and deterministic Elastic IP association (Sections~\ref{sec:ha-automation-aws} and \ref{sec:operational-considerations}).
...
\appendix
\section{Design Constraints and Lessons Learned}

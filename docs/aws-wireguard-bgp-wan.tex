...
\hline
Core Technologies & WireGuard, BGP (FRR), AWS EC2, EventBridge, Lambda, CloudWatch \\
\hline
VM Platform & AWS EC2 and VMware Workstation Pro: Ubuntu Server 24.04 LTS \\
\hline
...
\hline
1.0 & 2025-12-01 & Initial document draft \\
\hline
1.1 & 2025-12-14 & Validation alignment and terminology refinement \\
\hline
1.2 & 2026-01-25 & Raw data reconciliation and formatting standardization \\
\hline
1.3 & 2026-02-13 & Final editorial revision reflecting current lab state \\
\hline
\end{tabular}
...
% -----------------------------------------------------------------------------
% 6.4.2 AWS RESOURCES AND IDENTIFIERS
%
% Enumerates AWS resources that implement the HA control plane.
%
% Purpose:
%   - Establish traceability between documented design and deployed
%     infrastructure objects.
%   - Provide reproducibility guidance for equivalent lab environments.
%
% Sensitive identifiers and public endpoints are intentionally redacted.
% -----------------------------------------------------------------------------
...
%
% Provides operational evidence of EIP reassociation during failover
% and failback events.
%
% Log excerpts demonstrate:
%   - Deterministic EIP ownership transitions.
%   - Idempotent Lambda behavior (no-op when desired state satisfied).
%   - Successful execution of disassociate/associate API calls.
%
% Identifiers and public endpoints are intentionally redacted.
% -----------------------------------------------------------------------------
...
WireGuard does not perform routing decisions and does not dynamically resolve reachability. As demonstrated during the active/active hub experiment documented in Appendix~\ref{sec:appendix-design-constraints}, cryptographic constraints imposed by \texttt{AllowedIPs} override all routing protocol logic. When a destination prefix is not covered by a valid WireGuard peer association, traffic fails with errors such as:
...
These issues are documented in detail in Appendix~\ref{sec:appendix-design-constraints} and are intrinsic to WireGuard’s security model rather than misconfiguration errors.
